#!/usr/bin/env python3
"""
Deep Strategic SRS Review Script (Mode 2)
Comprehensive architectural and business-strategic evaluation with 10-dimension scoring.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deep Strategic SRS Review (Mode 2)")
    ...
    print(f"🔍 Running Deep Strategic Review (Mode 2)...\n")
    ...
    # 3. Load prompt
    prompt_path = project_root / "core" / "prompts" / "srs_strategic_prompt.txt"
    ...
    # 4. Run LLM review
    print("✓ Calling LLM for strategic 10-dimension scoring...")
    client = LLMClient()
    ...
    renderer = ReportRenderer(reviewer=args.reviewer, mode="Deep Strategic")
...
def print_summary(result: dict) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"📊 STRATEGIC REVIEW SUMMARY")
    print(f"{'='*60}")
